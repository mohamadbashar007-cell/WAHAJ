...
export function Hero() {
  ...
  return (
    <section id="top" className="hero" aria-labelledby="hero-title">
      <div className="hero-grid" aria-hidden="true" /><div className="hero-logo" aria-hidden="true" style={{ backgroundImage: `url(${assetPath('/wahaj-hero-mark.png')})` }} />
      ...
      <h1 id="hero-title" className="hero-title" aria-label={ar ? 'نصنع علامات تجارية متوهجة' : 'We make brands glow'}>
        <span>{ar ? 'نصنع' : 'WE MAKE'}</span><span>{ar ? 'علامات' : 'BRANDS'}</span><span className="glow">{ar ? 'تتوهج.' : 'GLOW.'}</span>
      </h1>
      <div className="hero-bottom reveal">
        ...
        <Link href={routeHref('/#work')} className="button button-yellow">{ar ? 'استعرض أعمالنا' : 'EXPLORE OUR WORK'} <span aria-hidden="true">↗</span></Link>
      </div>
      <Link href="#intro" className="scroll-cue">{ar ? 'مرّر للاستكشاف' : 'SCROLL TO EXPLORE'} <span aria-hidden="true">↓</span></Link>
    </section>
  )
}
